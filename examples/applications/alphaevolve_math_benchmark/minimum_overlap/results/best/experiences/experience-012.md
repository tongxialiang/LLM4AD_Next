该策略将低分辨率优化结果作为高分辨率优化的连续初始化，并在保持约束的同时进行局部精化。

- Two-Stage 96-to-128 Grid Continuation先保留m=64求解结果，在m=96网格上优化插值初始化，再将m=96的最佳半序列构造为完整反射序列并线性插值到m=128网格。
- Two-Stage 96-to-128 Grid Continuation在m=128初始化上执行值域裁剪、将非中心变量投影回平衡超平面、上皮求解器和主动滞后精化，并测试高斯噪声尺度0.0015、0.004和0.008的多个连续初始化。
- Two-Stage 96-to-128 Grid Continuation在m=64、m=96和m=128结果中逐一评估所有滞后并保留精确归一化最大相关性最小的候选；在固定中心值为0.5并满足平衡约束的m=128半序列上，观测到上界0.38092504473534605，低于目标0.380927。
