Reusable strategy for bottleneck geometric optimization under strict feasibility constraints.

- Projected SA with Worst-Triangle Gradient and Multi-Start Seeding: Use a smoothed bottleneck objective for simulated annealing acceptance, combining the true minimum triangle area with the mean of the K smallest areas, while tracking the unsmoothed minimum for best-so-far selection; in this run the method maintained validity=1.0. Parameterize points in barycentric coordinates with an epsilon interior margin and a light pairwise repulsion, projecting after each move to keep points strictly inside and distinct, which supported constraint satisfaction. Periodically apply a geometry-aware intensification that moves the vertices of the W worst triangles along area-increasing gradients in small projected steps, accepting only non-decreasing updates, to lift the active bottlenecks. Combine incremental evaluation that recomputes only triples touching the perturbed point with multi-start seeding from low-discrepancy and structured layouts; this run completed in eval_time=161.9368574639666 with min_area=0.033726609926134674 and target_ratio=0.9240167103050596.
