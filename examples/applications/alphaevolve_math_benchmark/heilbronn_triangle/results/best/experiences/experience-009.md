Heilbronn n=11: key design choices that worked and what to reuse

- Hybrid K‑Worst SA with Critical‑Triple Subgradient and Incremental Areas: Focusing proposals on a dynamically sized K‑worst triangle set and moving a selected vertex along the base’s outward normal, combined with strict barycentric projection and an incremental triangle‑area cache that updates only triangles incident to moved points, produced efficient, constraint‑safe gains; periodically aggregating critical‑triple subgradients and finishing with a deterministic polish lifted multiple co‑limiting triangles before narrowing to a single bottleneck, yielding min_area 0.033957 (target_ratio 0.930338, validity 1.0) in about 89.75 s; future designs should reuse this K‑worst guided normal‑move plus incremental‑update core, interleaving brief critical‑set subgradient sweeps as K decreases.
...
def run_search_point(n=11):
    ...
    def poisson_initial_points(npts, rng, eps=1e-6):
        ...
        while len(pts) < npts:
            ...
            for _ in range(candidates_per_step):
                ...
                dw = -du - dv
                u, v, w = clip_bary_interior(u + du, v + dv, w + dw, eps=eps)
                cx, cy = bary_to_cart(u, v, w)
    ...
    def anneal_single_restart(npts, rng_seed, init_mode="lattice"):
        ...
        # Subgradient critical set computation
        ...
        # One subgradient coordination step over critical triangles
        def subgradient_step(current_min_in, temp_scale, Tcur):
            # Collect critical triangles
            crit_tris = get_critical_triangles(current_min_in)
            if not crit_tris:
                return False, current_min_in

            # Aggregate gradients per point
            ...
            for ti in crit_tris:
                ...
                gx, gy = g[i]; g[i] = (gx + g1[0], gy + g1[1]); involved.add(i)
                gx, gy = g[j]; g[j] = (gx + g2[0], gy + g2[1]); involved.add(j)
                gx, gy = g[k]; g[k] = (gx + g3[0], gy + g3[1]); involved.add(k)
            ...
            # Sequential per-point updates along aggregated gradient (trust-region style)
            # Small step relative to base and temperature
            ...
            for idx in order:
                ...
                u, v, w = cart_to_bary(x0, y0)
                minb = max(0.0, min(u, v, w) - 1e-6)
                barrier_scale = 1.0
                if minb < barrier_thr:
                    barrier_scale = 0.5 + 0.5 * (minb / barrier_thr)
                step = grad_step * barrier_scale
                nx, ny = ensure_point_inside(x0 + step * ux, y0 + step * uy, eps=eps_inside)
                ox, oy = x0, y0
                # Apply and evaluate incrementally
                ...
                if new_min >= cur_min:
                    ...
                    prob = math.exp(delta / max(Tcur, 1e-12)) if Tcur > 0 else 0.0
                    if rng.random() < prob:
                        cur_min = new_min
                        accepted_any = True
                    else:
        ...
        for t in range(iterations):
            ...
            if rpick < p_targeted and worst_k:
                ...
                # Critical-set subgradient step (coordinated multi-point small moves)
