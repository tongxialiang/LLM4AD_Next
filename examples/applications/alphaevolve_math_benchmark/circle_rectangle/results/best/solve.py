...
def lp_refine(centers, iterations=15, fixed_bootstrap=True):
    ...
    # Fixed-center bootstrap
    if fixed_bootstrap:
        A_ub = np.zeros((num_constraints, num_vars))
        b_ub = np.zeros(num_constraints)
        
        bounds = []
        for i in range(N): bounds.append((centers[i, 0], centers[i, 0]))
        for i in range(N): bounds.append((centers[i, 1], centers[i, 1]))
        ...
        for i in range(N):
            for j in range(i+1, N):
                ...
                A_ub[row_idx, 2*N + i] = 1.0; A_ub[row_idx, 2*N + j] = 1.0
                row_idx += 1
...
def sa_ils_loop(centers, radii, W, H, initial_score, iterations=10):
    """Simulated Annealing Iterated Local Search."""
    best_centers, best_radii, best_W, best_H = centers.copy(), radii.copy(), W, H
    ...
    for _ in range(iterations):
        jitter = np.random.uniform(-1e-3, 1e-3, size=centers.shape)
        new_centers = best_centers + jitter
        
        c, r, w, h, s = lp_refine(new_centers, iterations=10, fixed_bootstrap=True)
        c, r, w, h, s = nlp_polish(c, r, w, h, epsilon=1e-10)
        
        if s > best_score:
            best_score = s
            best_centers, best_radii, best_W, best_H = c, r, w, h
            current_score = s
        elif np.exp((s - current_score) / T) > np.random.rand():
            current_score = s
            
        T *= alpha
            
    return best_centers, best_radii, best_W, best_H, best_score
...
def construct_packing(num_circles: int = 21):
    ...
    best_seeds = []
    for seed in seeds:
        centers, radii, W, H, score = lp_refine(seed, iterations=20, fixed_bootstrap=True)
        if score > 0:
    ...
    for score, centers, radii, W, H in top_5:
        centers, radii, W, H, score = lp_refine(centers, iterations=50, fixed_bootstrap=True)
        refined_seeds.append((score, centers, radii, W, H))
        
    refined_seeds.sort(key=lambda x: x[0], reverse=True)
    best_score, best_centers, best_radii, best_W, best_H = refined_seeds[0]
    ...
    for eps in eps_schedule:
        centers, radii, W, H, best_score = nlp_polish(centers, radii, W, H, epsilon=eps)
        
    # Phase 4: SA ILS loop
    centers, radii, W, H, best_score = sa_ils_loop(centers, radii, W, H, best_score, iterations=10)
