Two-phase hybrid optimization combining smooth L-BFGS-B continuation and non-smooth plateau polishing.

- Smooth Optimization Phase: The algorithm executes an alpha continuation schedule optimizing unconstrained softmax logits with FFT-accelerated L-BFGS-B on a LogSumExp surrogate, enforcing simplex constraints without penalty terms and rapidly converging to an initial basin.
- Plateau Polishing Engine: The algorithm projects logits to the probability simplex and runs a time-calibrated polishing loop that interleaves hard-max projected subgradient descent with cutting-plane steps over top-M peak averages.
- Fractional Backtracking Multi-Pair Shaving: The polishing phase shifts mass from the highest contributing pairs of peak autocorrelation indices to low-pressure positions using fractional scale backtracking (1.0, 0.5, 0.25, 0.125) and strictly accepts only moves that decrease the exact convolution objective to prevent plateau stalls.
