Mechanisms that yielded stable, monotone objective reductions and reliable performance under the 1000 s constraint.

- AW-PLUMAR initialization stack: AW-PLUMAR performs an arch-width sweep over s ∈ {0.7, 0.8, …, 1.3}, symmetrizes, clips to nonnegative, normalizes, evaluates F exactly, and then applies an immediate accepted-only peak-shaving on the best seed to secure a monotone decrease in F before gradient phases.
- Plateau-union active set in AW-PLUMAR: The plateau-union active set combines Top-K worst lags with a temperature-tied band P = {k : b[k] ≥ (1 − ε)·bmax} and neighbor expansion, and blends softmax weights with a tail of 0.5 uniform plus 0.5 inverse-pressure 1/(b − bmin + λ) to stabilize descent across near-tied maxima and prevent peak-swapping.
- Acceptance-tested non-smooth refinements in AW-PLUMAR: AW-PLUMAR interleaves pair-shaving transport, cutting-plane refinement of the average of the top-M lags, and hard-max projected subgradient polishing, accepting a change only when the exact objective F strictly decreases and backtracking when it does not.
- AW-PLUMAR performance metrics: On n = 600 with a ≥ 0 and sum(a) = 1, AW-PLUMAR achieved validity 1.0 within approximately 920.03 seconds and reported upper_bound 1.5095353 with target_ratio 0.9971943 and no errors.
- AW-PLUMAR-X algorithm: uses a dual-gated initialization that first performs a coarse arch-width sweep evaluated by the true evaluate_sequence, then applies a strictly accepted micro peak-shaving at the worst lag, and finally runs a fine sweep around the best width while re-normalizing on the L1-simplex to suppress the worst autoconvolution peaks before the gradient phase.
- AW-PLUMAR-X plateau-union minimax backbone: constructs surrogate weights over Top-K and plateau lags with neighbor expansion and an inverse-pressure tail, and optimizes softmax logits via Adam/AMSGrad with gradient clipping and learning-rate moderation on frequent argmax flips, which spreads descent pressure across near-tied maxima to avoid peak swapping instabilities.
- AW-PLUMAR-X non-smooth refinements: interleaves dual-gated peak transport, cutting-plane refinement, and hard-max polishing, each strictly acceptance-tested against the true objective and projected to the simplex, to secure monotonic decreases of F(a) in flat regions that smooth surrogates alone fail to improve.
- AW-PLUMAR-X implementation: computes convolution and correlation via FFT and maintains feasibility with a softmax parameterization (a ≥ 0, sum(a) = 1), continuously tracking and returning the best feasible sequence before an internal deadline (~980 s) under the 1000-second cap.
- AW-PLUMAR-X evaluation metrics: reported upper_bound 1.507459811737381, target_ratio 0.9985672508676091, validity 1.0, and eval_time 931.4216624699766 seconds, demonstrating compliance with constraints and efficient search within the time limit.
- Objective F(a): on the L1-simplex (sum(a) = 1) and fixed length n = 600, reduces to F(a) = 2·n·max(conv(a, a)), which AW-PLUMAR-X directly targets with FFT-accelerated autoconvolution.
...
def search_for_best_sequence():
    ...
    This version augments the initializer and optimizer with:
      1) Width-sweep arch seed selection over s in [0.7, 1.3].
      2) A tiny accepted-only peak-shaving transport on the best arch seed.
      3) Plateau-union active-set surrogate with inverse-pressure tail and adaptive Top-K.
      4) Interleaved hard-max polish and cutting-plane refinement, acceptance-tested.
      5) Symmetry mixing accepted only if it strictly improves the objective.
      6) Mild mass-transport "pair-shaving" steps gated by true objective improvement.
      7) Expanded diverse seed set (Kaiser, central rectangle, etc.).
      8) Final polishing continuation on the global best candidate.
    ...
    # Default to 980 seconds to utilize the full allowed time budget unless limited by the harness.
    ...
    def seeds_collection(n: int) -> List[np.ndarray]:
        ...
        a_arch_refined, _F_ref = accepted_peak_shaving(a_arch_best, max_iters=24, init_delta=2.5e-3, min_delta=8e-7)
