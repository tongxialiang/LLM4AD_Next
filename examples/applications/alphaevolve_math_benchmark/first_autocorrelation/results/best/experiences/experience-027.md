Algorithmic refinements combining continuation scheduling with broad-plateau non-smooth polishing to achieve a score of 0.995632.

- Extended L-BFGS-B continuation schedule: An extended 12-step continuation schedule reaching alpha = 10^7.5 provides tight initial smooth convergence on softmax logits before non-smooth polishing begins.
- Relative-threshold subgradient peak selection: Selecting active peaks using the relative threshold max_b * (1 - 1e-5) captures near-tied convolution peaks robustly across scale variations and avoids see-saw oscillations.
- Softmax-weighted cutting-plane refinement: Averaging descent directions across the top 32 peaks with softmax weighting stabilizes descent across wide plateaus and suppresses overshoot.
- Distributed multi-pair shaving: Shaving mass from the top 24 contributing pairs and distributing it evenly across the 20 lowest-gradient indices relieves peak pressure without creating new spurious peaks.
- Relative plateau band thresholding: Defining the active plateau band threshold relatively as max_b * (1 - 1e-5) in subgradient descent robustly captures near-tied convolution peaks regardless of scale variations.
- Expanded cutting-plane peak set: Increasing the number of targeted peak constraints M from 20 to 32 stabilizes cutting-plane descent across broad plateaus.
- Distributed mass shaving redistribution: Expanding multi-pair shaving to top_k=24 and distributing shaved mass evenly across the 20 lowest-gradient indices relieves peak pressure smoothly without introducing new spurious peaks, achieving an upper bound of 1.5119026907067235 and target ratio of 0.9956328600065941.
...
def search_for_best_sequence():
    ...
    while time.time() - start_time < 980:
        ...
        # Using relative threshold to robustly capture near-tied peaks
        active_indices = np.where(b >= max_b * (1 - 1e-5))[0]
        p = np.zeros_like(b)
        ...
        if iteration % 3 == 0:
            ...
            p_m = np.zeros_like(b)
            # Use relative temperature for softmax weighting
        ...
        if iteration % 5 == 0:
            ...
            if end_i >= start_i:
                ...
                # Shave mass from the top 24 contributing pairs
                top_k = min(24, len(contributions))
                if top_k > 0:
                    ...
                    # Redistribute evenly across the 20 lowest-gradient indices
                    num_low = min(20, len(grad))
