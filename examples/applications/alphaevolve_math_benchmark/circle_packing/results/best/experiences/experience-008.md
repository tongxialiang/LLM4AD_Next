Extend the monotone acceptance budget and slightly upweight inward boundary pushes to compound small gains in a deterministic LP-guided relaxation pipeline.

- MAX_ACCEPTS was increased from 120 to 160 to allow accumulation of small, non-decreasing gains produced by frequent micro-SLP (K=2) and longer small-gain patience (SMALL_IMPROV_STREAK_MAX=5), while maintaining deterministic monotone acceptance.
- BASE_W_BOUNDARY was raised from 2.0 to 2.3 to gently upweight inward boundary pushes, prioritizing slack creation for boundary-limited circles that the LP converts into larger radii, with safety preserved by weight-sum normalization, per-node capping, projection/clipping, and strictly non-decreasing objective acceptance.
- Algorithm 'Longer accept budget + slightly stronger boundary bias' achieved sum_radii 2.631093526681643 with validity 1.0 under these constant changes in solve.py, and made no other modifications to code paths or acceptance logic.
...
# Mutation: gently upweight inward boundary pushes to prioritize boundary-limited slack creation
BASE_W_BOUNDARY = 2.3  # base weight for boundary inward normal contributions (was 2.0)
...
# Acceptance limits
# Mutation: extend acceptance budget to allow compounding small gains
MAX_ACCEPTS = 160  # was 120
...
def construct_packing(num_circles: int = 26):
    ...
    Mutations integrated:
      ...
      - This iteration: increased acceptance budget (MAX_ACCEPTS=160) and upweighted boundary bias (BASE_W_BOUNDARY=2.3).
    """
    if num_circles != 26:
