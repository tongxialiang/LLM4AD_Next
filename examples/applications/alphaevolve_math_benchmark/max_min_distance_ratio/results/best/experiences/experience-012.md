Calipers shrink uses softmax-weighted active pairs with τ tied to tol_rel and per-node degree normalization to stabilize diameter reduction under multiple near-co-maximal chords.

- Softmax-Weighted Active-Pair Calipers (SWAP-Calipers): Replace uniform aggregation inside caliper_shrink_backtrack with a temperature-annealed softmax over active near-maximal pairs and degree-normalize point updates; tie the temperature τ to tol_rel so early iterations softly couple to more pairs and later iterations focus on the true worst chords. This produces smoother, coherent shrink directions that reduce active-set oscillations and backtracking stalls, improving acceptance rates on plateaus where several near-co-maximal antipodal chords coexist. Keep trust-radius clipping, project_feasible repair, light D_min normalization, and exact D_max backtracking unchanged to gate steps while benefiting from the stabilized aggregation; reuse this weighting-plus-degree-normalization scheme in future calipers-style diameter reduction loops under similar plateau conditions.
...
shrink with backtracking acceptance, MEC/hull guidance, ring-parameter
micro-CMA proposals, an affine major-axis squeeze, a safeguarded
tangential hull angular equalization, and a safeguarded antipodal
chord–shortening (ATCS) step.
...
2) Affine major-axis anisotropic squeeze:
...
3) Safeguarded tangential hull angular equalization:
   - Compute the exact MEC center and the convex hull in CCW order; represent hull points by their
     polar angles around the MEC center and apply a small tangential displacement that reduces
     angular gap imbalance using θ_k ← θ_k + β (Δ_{k−1} − Δ_k). The displacement is along the
     local circle tangent, trust-clipped, and accepted only if exact D_max does not increase
     after feasibility projection. This equalizes angular spacing on the hull, shortening several
     near-diameter chords simultaneously.
4) Safeguarded antipodal chord–shortening (ATCS):
   - Using the MEC center and convex hull, for each hull vertex i find a farthest antipodal partner
     a(i) on the hull. Restrict to pairs within a relative tolerance of the current D_max.
     For each selected pair (i, a), compute unit tangents at i and a with respect to the MEC center
     and apply small tangential displacements δx_i = −β_ap (u · t_i) t_i and δx_a = −β_ap (−u · t_a) t_a,
     where u is the unit vector along the chord from i to a. Aggregate and trust-clip displacements,
     repair feasibility, and accept only if the exact D_max does not increase. This directly shortens
     near-maximal antipodal chords and complements angular equalization.
...
- Final polish uses calipers shrink, MEC squeeze, angular equalization, ATCS,
  and several affine major-axis squeezes interleaved.
...
def optimize_construct(n=16, d=2, random_seed=42):
    ...
    def caliper_shrink_backtrack(X, eps=0.02, max_backtrack=12, tol_rel=None):
        ...
        step = float(eps)
        accepted = False
        # Temperature for softmax weighting tied to tol_rel (annealed); default small constant
        tau_default = 1e-3
        tau = tau_default if (tol_rel is None) else max(1e-8, 4.0 * float(tol_rel))
        dmax2 = float(dmax0 * dmax0)
        ...
        for _ in range(max_backtrack):
            ...
            # Compute softmax-like weights for active pairs based on squared distances
            # w_ij ∝ exp((s_ij - 1)/tau), s_ij = (d_ij^2 / dmax^2) ∈ (0, 1]
            w_raw = []
            dij2_list = []
            for (i, j) in pairs:
                vij = X0[j] - X0[i]
                dij2 = float(np.dot(vij, vij))
                dij2_list.append(dij2)
                s_ij = dij2 / max(dmax2, 1e-18)
                w_raw.append(np.exp((s_ij - 1.0) / tau))
            w_raw = np.array(w_raw, dtype=float)
            sumw = float(np.sum(w_raw))
            if not np.isfinite(sumw) or sumw <= 0.0:
                # Fallback to uniform weights in pathological case
                w = np.ones(len(pairs), dtype=float) / float(len(pairs))
            else:
                w = w_raw / sumw

            # Weighted displacement accumulation with degree-normalization by incident weight sums
            disp = np.zeros_like(X0)
            weight_incident = np.zeros(X0.shape[0], dtype=float)
            for k, (i, j) in enumerate(pairs):
                vij = X0[j] - X0[i]
                dij = float(np.linalg.norm(vij))
                if dij <= 1e-12:
                    continue
                u = vij / dij
                wij = float(w[k])
                # Move both endpoints towards the midpoint scaled by step and pair weight
                disp[i] += step * wij * u
                disp[j] -= step * wij * u
                weight_incident[i] += wij
                weight_incident[j] += wij

            # Degree-normalize per node by sum of incident weights to avoid overshrinking high-degree nodes
            for idx in range(X0.shape[0]):
                wi = float(weight_incident[idx])
                if wi > 0.0:
                    disp[idx] /= wi
    ...
    # ------------------------------
    # Tangential hull angular equalization micro-step
    # ------------------------------
    def _wrap_to_2pi(a):
        # Wrap angle to [0, 2π)
        return float(np.mod(a, 2.0 * pi))

    def hull_angular_equalize_step(X, beta=0.02, trust=0.02, prev_dmax=None, focus_top_k=None):
        """Safeguarded tangential re-spacing of hull points along the exact MEC.

        Steps:
        - Compute exact MEC center c* and convex hull indices in CCW order.
        - For each hull point k, compute its polar angle θ_k around c*.
        - Angular gaps Δ_k = mod(θ_{k+1} − θ_k, 2π).
        - Update angles by δθ_k = beta · (Δ_{k-1} − Δ_k), a discrete Laplacian-like smoothing.
        - Apply tangential displacement δx_k = R_k · δθ_k · t_k, where R_k = ||x_k − c*|| and
          t_k is the unit tangent at x_k relative to c*.
        - Clip ||δx_k|| ≤ trust. Non-hull points are unchanged.
        - Repair feasibility and accept only if exact D_max does not increase.

        Optionally, focus on top-K largest gaps by scaling local (Δ_{k-1} − Δ_k).
        """
        X0 = X.copy()
        if prev_dmax is None:
            _, _, prev_dmax = farthest_pair_indices(X0)
        ...
        if h <= 1:
            return X0, False
        # Polar angles of hull points about MEC center
        angles = []
        radii = []
        for idx in hull:
            v = X0[idx] - c_star
        ...
        # Angular gaps (wrapped to [0, 2π))
        gaps = np.zeros(h, dtype=float)
        for k in range(h):
            dth = angles[(k + 1) % h] - angles[k]
            gaps[k] = _wrap_to_2pi(dth)

        # Optional focus on top-K largest gaps: emphasize boundaries k and k-1
        weights = np.ones(h, dtype=float)
        if focus_top_k is not None and focus_top_k > 0 and h >= 3:
            K = int(min(max(1, focus_top_k), h))
            top_idx = np.argsort(-gaps)[:K]
            for tk in top_idx:
                weights[tk] *= 1.6
                weights[(tk - 1) % h] *= 1.6

        # δθ_k = beta * w_k * (Δ_{k-1} - Δ_k)
        delta_theta = np.zeros(h, dtype=float)
        for k in range(h):
            delta_theta[k] = beta * weights[k] * (gaps[(k - 1) % h] - gaps[k])

        # Build tangential displacements for hull vertices
        disp = np.zeros_like(X0)
        for k in range(h):
            idx = hull[k]
            th = angles[k]
            Rk = radii[k]
            # Unit tangent at angle th: t = [-sin(th), cos(th)]
            t = np.array([-sin(th), cos(th)], dtype=float)
            delta = Rk * delta_theta[k] * t
            dn = float(np.linalg.norm(delta))
            if dn > trust:
                delta = delta * (trust / dn)
            disp[idx] += delta
    ...
    # ------------------------------
    # Antipodal tangential chord–shortening micro-step (ATCS)
    # ------------------------------
    def antipodal_tangential_chord_shorten_step(X, beta_ap=0.01, trust=0.04, tol_rel_ap=1e-3, prev_dmax=None):
        """Safeguarded antipodal chord–shortening:
        - Find MEC center c* and the convex hull.
        - For each hull vertex i, find an antipodal partner a(i) on the hull that maximizes ||xi - xj||.
          Build the set of unique pairs (i, a(i)) restricted to those within relative tolerance of D_max.
        - For each selected pair (i, j), compute u = (xj - xi)/||...|| and the unit tangents at endpoints
          t_i, t_j about c*. Apply δx_i = -beta_ap * (u·t_i) t_i and δx_j = -beta_ap * ((-u)·t_j) t_j.
        - Aggregate displacements per point (averaging if necessary), trust-clip, project feasibility.
        - Accept only if the exact hull D_max does not increase.
        """
        X0 = X.copy()
        # Baseline D_max for acceptance
        if prev_dmax is None:
            _, _, prev_dmax = farthest_pair_indices(X0)
        # MEC center
        c_star, _, _ = mec_circle(X0)
        ...
        # Compute all pair distances over hull and keep D_max^2
        dmax2 = -1.0
        H = [int(idx) for idx in hull]
        # Precompute pairwise squared distances for hull
        hull_coords = X0[H]
        # Determine farthest partner for each hull vertex (brute force; small h)
        farthest_partner = {}
        for ai, i in enumerate(H):
            bestj = None
            bestd2 = -1.0
            xi = hull_coords[ai]
            for aj, j in enumerate(H):
                if i == j:
                    continue
                d2 = float(np.dot(hull_coords[aj] - xi, hull_coords[aj] - xi))
                if d2 > bestd2:
                    bestd2 = d2
                    bestj = j
            farthest_partner[i] = (bestj, bestd2)
            if bestd2 > dmax2:
                dmax2 = bestd2
        if dmax2 <= 0.0:
            return X0, False

        # Select near-max antipodal pairs within relative tolerance
        thr2 = (1.0 - max(0.0, float(tol_rel_ap))) * dmax2
        selected = set()
        for i in H:
            j, d2 = farthest_partner.get(i, (None, None))
            if j is None:
                continue
            if d2 >= thr2 - 1e-15:
        ...
        # Accumulate tangential displacements for endpoints
        disp = np.zeros_like(X0)
        ...
        def unit_tangent(x):
            ...
            if r <= 1e-12:
                return np.zeros(2), 0.0
            # Unit tangent by 90° rotation of radial unit vector
            vr = v / r
            t = np.array([-vr[1], vr[0]], dtype=float)
            return t, r
        ...
        for (i, j) in selected:
            ...
            if dij <= 1e-12:
                continue
            u = vij / dij
            ti, ri = unit_tangent(X0[i])
            tj, rj = unit_tangent(X0[j])
            # Local tangential components along chord direction
            # For i: reduce chord length => move in direction - (u·ti) ti
            # For j: u_a = -u, so -(u_a·tj) tj = -((-u)·tj) tj = (u·tj) tj with a minus sign applied below
            # We stick to the symmetric form from description:
            delt_i = -beta_ap * float(np.dot(u, ti)) * ti
            delt_j = -beta_ap * float(np.dot(-u, tj)) * tj
            # Trust-clip each local contribution (per pair) to avoid rare large steps
            n_i = float(np.linalg.norm(delt_i))
            if n_i > trust:
                delt_i = delt_i * (trust / n_i)
            n_j = float(np.linalg.norm(delt_j))
            if n_j > trust:
                delt_j = delt_j * (trust / n_j)
            disp[i] += delt_i
            disp[j] += delt_j
            counts[i] += 1
            counts[j] += 1

        # Average displacements per point and global trust-clip
        for idx in range(X0.shape[0]):
            c = counts[idx]
            if c > 0:
                disp[idx] /= float(c)
                n = float(np.linalg.norm(disp[idx]))
                if n > trust:
                    disp[idx] = disp[idx] * (trust / n)
    ...
    class Island:
        ...
        def _step_A(self, t_global, total_steps):
            # Smooth-Projected Caliper Descent + ANGULAR EQUALIZATION + ATCS + affine major-axis squeeze + MEC support squeeze
            X = self.X.copy()
            ...
            beta_eq = self._alpha_schedule(self.iter, max(1, total_steps), 0.020, 0.006)
            beta_ap = self._alpha_schedule(self.iter, max(1, total_steps), 0.015, 0.004)
            ...
            # Tangential hull angular equalization (safeguarded), focus top-K gaps every other iter
            _, _, dmax_prev = farthest_pair_indices(X)
            focus_k = 3 if (self.iter % 2) == 0 else None
            X_eq, ok_eq = hull_angular_equalize_step(X, beta=beta_eq, trust=trust, prev_dmax=dmax_prev, focus_top_k=focus_k)
            if ok_eq:
                X = X_eq

            # Antipodal tangential chord–shortening (safeguarded), applied every other iteration
            if (self.iter % 2) == 0:
                _, _, dmax_prev = farthest_pair_indices(X)
                X_ap, ok_ap = antipodal_tangential_chord_shorten_step(
                    X, beta_ap=beta_ap, trust=trust, tol_rel_ap=tol_rel, prev_dmax=dmax_prev
                )
                if ok_ap:
                    X = X_ap
        ...
        def _step_B(self, t_global, total_steps):
            # MEC/Hull-guided contraction with repulsions, calipers shrink, ANGULAR EQUALIZATION, ATCS,
            # MEC support squeeze, and affine major-axis squeeze
            X = self.X.copy()
            ...
            # Tangential hull angular equalization (safeguarded), focus top-K gaps every third iter
            _, _, dmax_prev = farthest_pair_indices(X)
            focus_k = 3 if (self.iter % 3) == 0 else None
            X_eq, ok_eq = hull_angular_equalize_step(X, beta=beta_eq, trust=trust, prev_dmax=dmax_prev, focus_top_k=focus_k)
            if ok_eq:
                X = X_eq

            # Antipodal tangential chord–shortening (safeguarded) every second iteration
            if (self.iter % 2) == 0:
                _, _, dmax_prev = farthest_pair_indices(X)
                X_ap, ok_ap = antipodal_tangential_chord_shorten_step(
                    X, beta_ap=beta_ap, trust=trust, tol_rel_ap=tol_rel, prev_dmax=dmax_prev
    ...
    # Helper: topology restart from elite
    ...
    # Final elite polish: feasibility pass + calipers shrinks + ANGULAR EQUALIZATION + ATCS + MEC squeeze + affine squeezes
    ...
    for it in range(60):
        ...
        # Tangential hull angular equalization interleaved
        _, _, dmax_prev = farthest_pair_indices(final)
        beta_polish = 0.007 * (1.0 - 0.3 * (it / 60.0))
        final_eq, ok_eq = hull_angular_equalize_step(final, beta=beta_polish, trust=0.02, prev_dmax=dmax_prev, focus_top_k=None)
        if ok_eq:
            final = final_eq

        # Interleave ATCS every ~6 passes
        if (it % 6) == 3:
            _, _, dmax_prev = farthest_pair_indices(final)
            beta_ap_polish = 0.006
            final_ap, ok_ap = antipodal_tangential_chord_shorten_step(
                final, beta_ap=beta_ap_polish, trust=0.02, tol_rel_ap=5e-4, prev_dmax=dmax_prev
            )
            if ok_ap:
                final = final_ap
