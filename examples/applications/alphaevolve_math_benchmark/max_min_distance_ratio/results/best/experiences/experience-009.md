Conservative tangential equalization of convex-hull angles around the MEC, scheduled between caliper shrink and anisotropic squeeze, with monotone D_max acceptance and annealed step size.

- hull_angular_equalize_step: computes the exact Minimum Enclosing Circle center and radius and the convex hull in counterclockwise order, derives polar angles and angular gaps for hull points, and updates angles with a discrete Laplacian smoothing θ_k^cand = θ_k + beta·(Δ_{k−1} − Δ_k) to generate small tangential displacements along the circle; non-hull points receive no move.
- hull_angular_equalize_step: clips each tangential displacement to a trust radius between approximately 0.02 and 0.06 and uses an annealed beta schedule from 0.020 to 0.006 to keep the step conservative.
- hull_angular_equalize_step: accepts a candidate configuration only if the exact convex-hull D_max after project_feasible() does not increase beyond prev_dmax + 1e−12, preserving monotone non-increase of D_max (and thus R^2 once D_min == 1).
- MEC-Centered Hull Angular Equalization Micro-step: targets plateaus for n=16 where the hull has uneven angular spacing that correlates with multiple near-maximal antipodal distances, and by redistributing hull points more uniformly along the MEC it shortens several near-diameter chords simultaneously and lowers D_max.
- Integration details for hull_angular_equalize_step: insert the step after caliper_shrink_backtrack() and before affine_major_axis_squeeze() in Island A and Island B, optionally apply it every 2–3 iterations focusing on the top-K largest gaps (K≈3), and in final polish interleave 4–6 passes with beta ≈ 0.006–0.008 and trust ≈ 0.02.
- Generation 8: achieved ratio_squared 12.899980793208005 with validity 1.0, score 0.9991694033208447, and eval_time 183.10949478700059.
...
micro-CMA proposals, an affine major-axis squeeze, and a safeguarded
tangential hull angular equalization.
...
- Final polish uses calipers shrink, MEC squeeze, angular equalization, and several
...
def optimize_construct(n=16, d=2, random_seed=42):
    ...
    class Island:
        ...
        def _step_A(self, t_global, total_steps):
            # Smooth-Projected Caliper Descent + ANGULAR EQUALIZATION + affine major-axis squeeze + MEC support squeeze
            X = self.X.copy()
        ...
        def _step_B(self, t_global, total_steps):
            # MEC/Hull-guided contraction with repulsions, calipers shrink, ANGULAR EQUALIZATION,
    ...
    # Final elite polish: feasibility pass + calipers shrinks + ANGULAR EQUALIZATION + MEC squeeze + affine squeezes
