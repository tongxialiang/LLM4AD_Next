Insight from EM-SQP-MTBH run that met the 16-point 2D target with ratio_squared ≈ 12.889 and validity 1.0.

- Exact Minimax SQP with Memetic Topological Basin Hopping (EM-SQP-MTBH): Pairing an exact SLSQP solver that uses a fully vectorized analytical Jacobian for the minimax distance reformulation with a memetic basin-hopping outer loop seeded by 34 ring-packing topologies and random initializations—plus feasibility projection before each SQP refinement—enabled escape from topological local minima and delivered ratio_squared ≈ 12.889 (score ≈ 1.0000028, validity 1.0) on n=16, d=2; future designs encountering plateaued minima should reuse this combination of exact-Jacobian SQP for fast local convergence with topology-diverse restarts and projection-to-feasibility before refinement.
...
def optimize_construct(n=16, d=2):
    ...
    # Evaluate all topological priors
